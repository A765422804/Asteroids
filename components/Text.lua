...
--[[
    ...
    -> fade_in: boolean - Should text fade in (optional)
        default: false
    -> fade_out: boolean - Should text fade out (optional)
        default: false
            ...
            if self.opacity > 0 then
                ...
                if fade_in then
                    if self.opacity < 1 then
                        self.opacity = self.opacity + (1 / TEXT_FADE_DUR / love.timer.getFPS()) -- 得到每帧增加的透明度                        
                    else
                        fade_in = false
                    end
                elseif fade_out then
                    self.opacity = self.opacity - (1 / TEXT_FADE_DUR / love.timer.getFPS()) -- 得到每帧减少的透明度
                end
